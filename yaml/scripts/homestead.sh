...
server ${dns_ip}
update add homestead-0.example.com. 30 A $(ip addr show dev eth0 | grep "inet\b" | awk '{print $2}' | cut -d/ -f1)
update add hs.example.com. 30 A $(ip addr show dev eth0 | grep "inet\b" | awk '{print $2}' | cut -d/ -f1)
send
EOF
